...
for (size_t i = 0; i < count; i++) {
    ...
    *queue_ptrs[index]++ = value;
    __builtin_prefetch(queue_ptrs[index]); // <-- the magic happens here
}
```

That's it. We prefetch the next position in the queue after writing to the queue. 87.5% of the time this does nothing, since the next position is in the same cache line (a 64-byte cache line holds 8 elements, so the next element falls into the same cache line 7 out of 8 times).
