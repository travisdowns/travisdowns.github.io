...
You might notice that Ice Lake, Intel's newest microarchitecture, is missing from this list: that's because there is a [whole separate post]({{ site.baseurl }}{% post_url 2020-05-18-icelake-zero-opt %}) on it.
...
Of course, the fact that this seems to currently only apply on Skylake and [Ice Lake]({{ site.baseurl }}{% post_url 2020-05-18-icelake-zero-opt %}) client hardware makes specifically targeting this quite dubious indeed.
